...
interface KlineQualityReport {
  ...
  checks: {
    ...
    uniqueness: {
      ...
      duplicate_details?: DuplicateGroup[];
    };
    coverage: {
      status: string;
            ...
            <Space size="large" wrap style={{ margin: '16px 0' }}>
              ...
              <div className="check-item">
                ...
                <Text
                  style={{
                    marginLeft: 8,
                    color: qualityReport.checks.continuity.missing_records > 0 ? 'red' : 'black',
                  }}
                >
          ...
          <div className="check-section">
            ...
            <Space direction="vertical" size="middle" style={{ width: '100%' }}>
              <div className="coverage-item">
                ...
                <Tag
                  color={qualityReport.checks.coverage.missing_historical_data ? 'red' : 'green'}
                  style={{ marginLeft: 8 }}
                >
              ...
              <div className="coverage-item">
                ...
                <Tag
                  color={qualityReport.checks.coverage.missing_future_data ? 'red' : 'green'}
                  style={{ marginLeft: 8 }}
                >
            ...
            <Space size="large" wrap style={{ margin: '16px 0' }}>
              <div className="check-item">
                ...
                <Text
                  style={{
                    marginLeft: 8,
                    color: qualityReport.checks.validity.total_invalid_records > 0 ? 'red' : 'black',
                  }}
                >
                ...
                <Table
                  columns={[
                    { title: '时间', dataIndex: 'timestamp', key: 'timestamp' },
                    {
                      ...
                      key: 'change_pct',
                      render: (text: number) => (
                        <Text style={{ color: text > 0 ? 'red' : 'green' }}>{text.toFixed(2)}</Text>
                      ),
                    },
                  ]}
            ...
            <Space size="large" wrap style={{ margin: '16px 0' }}>
              <div className="check-item">
                ...
                <Text
                  style={{
                    marginLeft: 8,
                    color: qualityReport.checks.uniqueness.duplicate_records > 0 ? 'red' : 'black',
                  }}
                >
            ...
            {qualityReport.checks.uniqueness.duplicate_records > 0 && (
              <div className="issue-section" style={{ marginTop: 16 }}>
                <Space size="middle" wrap>
                  <Button type="primary" onClick={fetchDuplicateDetails} loading={isLoadingDuplicates}>
                    查看重复记录详情
                  </Button>
                  <Button
                    danger
                    onClick={() => {
                      Modal.confirm({
                        title: '确认处理所有重复记录',
                        content: `确定要使用「${
                          selectedStrategy === 'keep_first'
                            ? '保留第一条记录'
                            : selectedStrategy === 'keep_last'
                              ? '保留最后一条记录'
                              : selectedStrategy === 'keep_max_volume'
                                ? '保留成交量最大的记录'
                                : '保留成交量最小的记录'
                        }」策略处理所有重复记录吗？`,
                        okText: '确定',
              ...
              <div className="duplicate-details" style={{ marginTop: 24 }}>
                ...
                <Card size="small" style={{ marginBottom: 16 }}>
                  <Space size="middle">
                    ...
                    <Text strong>
                      总重复记录数: {duplicateDetails.reduce((sum, group) => sum + group.count, 0)}
                    </Text>
                  </Space>
                </Card>
                {/* 筛选表单 */}
