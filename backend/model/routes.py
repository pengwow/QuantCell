            ...
            "content": {
                "application/json": {
                    "example": {
                        "code": 0,
                        "message": "获取模型列表成功",
                        "data": {"models": ["xgboost", "catboost", "random_forest"]},
                    }
                }
            },
        },
...
def get_model_list(request: ModelListRequest = None):
    ...
    try:
        ...
        return ApiResponse(code=0, message="获取模型列表成功", data=result)
...
def train_model(request: ModelTrainRequest):
    ...
    try:
        ...
        return ApiResponse(code=0, message="模型训练成功", data=result)
    except Exception as e:
...
def evaluate_model(request: ModelEvaluateRequest):
    ...
    try:
        ...
        # 评估模型
        result = model_service.evaluate_model(model_name=request.model_name, dataset_config=request.dataset_config)

        logger.info(f"模型评估完成，结果: {result}")

        return ApiResponse(code=0, message="模型评估成功", data=result)
    except Exception as e:
...
def predict(request: ModelPredictRequest):
    ...
    try:
        ...
        # 模型预测
        result = model_service.predict(model_name=request.model_name, data=request.data)

        logger.info(f"模型预测完成，结果: {result}")

        return ApiResponse(code=0, message="模型预测成功", data=result)
    except Exception as e:
...
def create_model_config(request: ModelConfigRequest):
    ...
    try:
        ...
        # 创建模型配置
        model_config = model_service.create_model_config(model_type=request.model_type, params=request.params)

        if model_config:
            logger.info(f"模型配置创建成功，模型类型: {request.model_type}")
            return ApiResponse(code=0, message="模型配置创建成功", data={"model_config": model_config})
        else:
            logger.error(f"模型配置创建失败，模型类型: {request.model_type}")
            return ApiResponse(code=1, message="模型配置创建失败", data={})
    except Exception as e:
...
def get_model_config(model_name: str):
    ...
    try:
        ...
        if model_config:
            ...
            return ApiResponse(code=0, message="模型配置获取成功", data={"model_config": model_config})
        else:
            logger.warning(f"模型配置获取失败，模型名称: {model_name}")
            return ApiResponse(code=1, message="模型配置获取失败", data={})
    except Exception as e:
