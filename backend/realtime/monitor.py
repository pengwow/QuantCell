...
class RealtimeMonitor:
    ...
    def record_message(self, data_type: str, success: bool, processing_time: float = 0) -> None:
        ...
        if processing_time > 0:
            ...
            self.stats["average_processing_time"] = sum(self.stats["processing_times"]) / len(
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "connections": {
                ...
                else self.stats["successful_connections"] / self.stats["total_connections"],
            },
            # 消息统计
            "messages": {
                ...
                else self.stats["processed_messages"] / self.stats["total_messages"],
            ...
            "performance": {
                ...
                "min_processing_time": self.stats["min_processing_time"] * 1000
    ...
    def _output_monitor_report(self, stats: dict[str, Any]) -> None:
        ...
        logger.info(
            f"连接统计: 总数={connections['total']}, 成功={connections['successful']}, 失败={connections['failed']}, 成功率={connections['success_rate']:.2%}"
        )

        # 消息统计
        messages = stats["messages"]
        logger.info(
            f"消息统计: 总数={messages['total']}, 处理成功={messages['processed']}, 处理失败={messages['failed']}, 成功率={messages['success_rate']:.2%}"
        )
        ...
        logger.info(
            f"性能统计: 平均处理时间={performance['average_processing_time']:.2f} 毫秒, 最大={performance['max_processing_time']:.2f} 毫秒, 最小={performance['min_processing_time']:.2f} 毫秒"
        )
        ...
        if data_type_stats:
            ...
            for data_type, data_stats in data_type_stats.items():
                ...
                logger.info(
                    f"  {data_type}: 总数={data_stats['total']}, 成功={data_stats['successful']}, 失败={data_stats['failed']}, 成功率={success_rate:.2%}"
                )
        ...
        if consumer_stats:
            ...
            for consumer_name, consumer_data in consumer_stats.items():
                ...
                logger.info(
                    f"  {consumer_name}: 总数={consumer_data['total']}, 成功={consumer_data['successful']}, 失败={consumer_data['failed']}, 成功率={success_rate:.2%}"
                )

        logger.info("====================")

    def get_alert(self) -> dict[str, Any] | None:
        """
        获取告警信息
