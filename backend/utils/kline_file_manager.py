...
from utils.parquet_utils import append_to_parquet, load_from_parquet, save_to_parquet
...
class KlineFileManager:
    ...
    def _get_file_path(self, symbol: str, interval: str, date_str: str, market_type: str = "spot") -> Path:
        """
        获取Parquet文件的完整路径
        ...
        Returns:
            Path: 文件完整路径
        """
        return self.base_dir / "crypto" / market_type / "klines" / symbol / interval / f"{date_str}.parquet"
    ...
    def _ensure_dataframe_format(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not all(col in df.columns for col in required_cols):
            msg = f"DataFrame缺少必需列: {required_cols}"
    ...
    def save_klines(self, df: pd.DataFrame, symbol: str, interval: str, market_type: str = "spot") -> bool:
        """
        保存K线数据到Parquet文件
        ...
        try:
            ...
            if "timestamp" in df.columns:
                ...
                logger.info(
                    f"[KlineFileManager] 保存成功: {symbol} {interval}, 共{len(df)}条数据, 分{success_count}个文件"
                )
    ...
    def load_klines(
        ...
        try:
            ...
            if start_time or end_time:
                ...
                # 解析开始时间的月份
                start_month = None
                if start_time and isinstance(start_time, str) and len(start_time) >= 7:
                    start_month = start_time[:7]

                # 解析结束时间的月份
                end_month = None
                if end_time and isinstance(end_time, str) and len(end_time) >= 7:
            ...
            logger.info(f"[KlineFileManager] 加载成功: {symbol} {interval}, 共{len(combined_df)}条数据")
    ...
    def append_klines(self, df: pd.DataFrame, symbol: str, interval: str, market_type: str = "spot") -> bool:
        """
        追加K线数据到现有文件
    ...
    def get_date_range(self, symbol: str, interval: str, market_type: str = "spot") -> tuple:
        """
        获取数据的日期范围
    ...
    def delete_klines(self, symbol: str, interval: str, market_type: str = "spot") -> bool:
        """
        删除指定交易对的K线数据
    ...
    def get_storage_stats(self) -> dict[str, Any]:
        ...
        for market_type in ["spot", "future"]:
            ...
            for symbol_dir in market_dir.iterdir():
                ...
                symbol_stats = {"intervals": {}, "total_size_mb": 0.0, "total_files": 0}
