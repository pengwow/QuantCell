...
class StrategyASTParser:
    ...
    def __init__(self, strategy_bases: list[str] | None = None, config_suffix: str | None = None):
        """
        初始化AST解析器
    ...
    def parse(self, code: str) -> ast.AST | None:
        """
        解析Python代码为AST
    ...
    def _is_strategy_class(self, class_name: str, base_classes: list[str]) -> bool:
        ...
        # 检查基类是否为策略基类
        return any(base_class in self.strategy_bases for base_class in base_classes)
    ...
    def _has_external_trading_import(self, tree: ast.AST) -> bool:
        ...
        return any("trader" in imp or "_trader" in imp for imp in imports)
    ...
    def is_axon_strategy(self, tree: ast.AST) -> bool:
        ...
        return any("axon_quant" in imp or "axon" in imp.lower() for imp in imports)

    def extract_params_from_config_class(
        self, config_class_node: ast.ClassDef, file_content: str
    ) -> list[dict[str, Any]]:
        """
    ...
    def extract_params_from_strategy_class(
        self, strategy_class_node: ast.ClassDef, file_content: str
    ) -> list[dict[str, Any]]:
        """
