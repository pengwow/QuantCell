...
class ConfigManager:
    ...
    def _set_nested_value_with_description(container: dict, key: str, value: Any, description: str = ""):
        ...
        last_part = parts[-1]
        if description:
            current[last_part] = {"_value": value, "_description": description}
        else:
    ...
    def import_from_toml(
        input_path: str | None = None, overwrite: bool = True, dry_run: bool = False
    ) -> dict[str, Any]:
        """从TOML文件导入系统配置
        ...
        try:
            ...
            if not dry_run:
                db.commit()
                logger.info(
                    f"配置导入完成: 总计={stats['total']}, 新建={stats['created']}, "
    ...
    def from_json(self, json_str: str) -> bool:
        ...
        try:
            ...
            logger.error(f"JSON加载配置失败: {e!s}")
            return False

    def json_to_toml(self, json_str: str, output_path: str | None = None) -> str | None:
        """将JSON字符串转换为TOML文件
        ...
        try:
            ...
            # 确定输出路径
            toml_path = self.config_path if output_path is None else Path(output_path)
            ...
            logger.error(f"JSON转TOML失败: {e!s}")
            return None

    def toml_to_json(self, input_path: str | None = None) -> str:
        """将TOML文件转换为JSON字符串
        ...
        try:
            # 确定输入路径
            toml_path = self.config_path if input_path is None else Path(input_path)
            ...
            logger.error(f"TOML转JSON失败: {e!s}")
            return "{}"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--include-sensitive", action="store_true", help="导出时包含敏感配置")
    parser.add_argument("--overwrite", action="store_true", default=True, help="导入时覆盖已存在的配置")
