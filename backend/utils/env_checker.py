...
class EnvChecker:
    ...
    def check_python_version() -> tuple[bool, str]:
        ...
        if current_version >= required_version:
            return (
                True,
                f"Python {current_version[0]}.{current_version[1]} (要求 >= {required_version[0]}.{required_version[1]})",
            )
        else:
            return (
                False,
    ...
    @staticmethod
    def check_config_file(config_path: str | None = None) -> tuple[bool, str]:
        """检查配置文件
    ...
    def check_all() -> dict[str, any]:
        ...
        required_packages_ok = all(p["installed"] for p in results["packages"]["required"].values())

        results["overall"] = python_ok and required_packages_ok and db_ok
    ...
    def print_report(results: dict) -> None:
        ...
        # Python版本
        "✓" if results["python"]["ok"] else "✗"

        # 依赖包
        for info in results["packages"]["required"].values():
            "✓" if info["installed"] else "✗"
            f" ({info['version']})" if info["version"] else ""

        for info in results["packages"]["optional"].values():
            "✓" if info["installed"] else "○"
            f" ({info['version']})" if info["version"] else ""

        # 数据库
        "✓" if results["database"]["ok"] else "✗"

        # 配置文件
        "✓" if results["config"]["ok"] else "○"
