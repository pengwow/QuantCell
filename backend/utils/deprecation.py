    ...
    def decorator(func_or_class):
        if isinstance(func_or_class, type):
            return _deprecate_class(
                func_or_class, version, remove_version, 
                replacement, category, stacklevel
            )
        else:
...
def _deprecate_function(
    ...
    def wrapper(*args, **kwargs):
        msg = _build_message(
            func.__qualname__, version, remove_version, replacement
        )
        warnings.warn(msg, category, stacklevel=stacklevel)
        _log_deprecation(msg)
        return func(*args, **kwargs)
    
    wrapper.__doc__ = _update_docstring(
        func.__doc__, version, remove_version, replacement
    )
    return wrapper
...
def _deprecate_class(
    ...
    def new_init(self, *args, **kwargs):
        msg = _build_message(
            cls.__qualname__, version, remove_version, replacement
        )
        warnings.warn(msg, category, stacklevel=stacklevel)
        _log_deprecation(msg)
    ...
    cls.__doc__ = _update_docstring(
        cls.__doc__, version, remove_version, replacement
    )
    return cls
...
def deprecated_property(
    ...
    wrapper.__doc__ = _update_docstring(
        getter_func.__doc__, version, None, replacement
    )
    return wrapper
...
def deprecated_compat(new_api: str = None, version: str = "2.1"):
    """
    ...
    def decorator(func):
        ...
        wrapper.__doc__ = _update_docstring(
            func.__doc__, version, None, new_api or "新的事件驱动 API"
        )
        return wrapper
    return decorator
