...
def parse_time_range(time_range: str | None) -> tuple[datetime | None, datetime | None]:
    ...
    if len(parts) != 2:
        msg = f"时间范围格式错误: {time_range}，应为 YYYYMMDD-YYYYMMDD"
    ...
    if start_date >= end_date:
        msg = f"开始日期必须早于结束日期: {start_date} >= {end_date}"
        raise ValueError(msg)
...
def timestamp_to_datetime(ts: int | float, unit: str = "ms") -> datetime:
    """
    时间戳转日期时间
...
def get_time_range_for_download(
    start_time: datetime, end_time: datetime, buffer_days: int = 1
