...
class SystemConfigItem(BaseModel):
    ...
    name: str | None = Field(
        None,
        description="配置名称，用于区分系统配置页面的子菜单名称",
        examples=["数据配置"],
    )
    is_sensitive: bool = Field(
        default=False,
        description="是否敏感配置，敏感配置API不返回真实值",
        examples=[False],
    )
    created_at: str | None = Field(None, description="创建时间", examples=["2026-01-24 18:00:00"])
    updated_at: str | None = Field(None, description="更新时间", examples=["2026-01-24 18:00:00"])
...
class ConfigUpdateRequest(BaseModel):
    ...
    description: str | None = Field(None, description="配置项描述", examples=["新的配置项"])
    plugin: str | None = Field(None, description="插件名称")
    name: str | None = Field(None, description="配置名称", examples=["基础配置"])
...
class ConfigBatchUpdateItem(BaseModel):
    ...
    description: str | None = Field(None, description="配置项描述", examples=["配置项1"])
    plugin: str | None = Field(None, description="插件名称")
    name: str | None = Field(None, description="配置名称", examples=["基础配置"])
    is_sensitive: bool = Field(default=False, description="是否为敏感配置", examples=[False])
...
class ConfigBatchUpdateRequest(BaseModel):
    ...
    configs: dict[str, str] | list[ConfigBatchUpdateItem] = Field(
        ...,
