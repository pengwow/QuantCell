...
def _check_plots_structure(code: str) -> List[Dict[str, Any]]:
    ...
    if not re.search(r"['\"]plots['\"]", code):
        hints.append({"severity": "error", "code": "MISSING_PLOTS_KEY", "message": "output 字典中缺少 'plots' 键"})
    elif re.search(r"['\"]plots['\"]\s*:\s*\[\s*\]", code):
        hints.append({"severity": "warn", "code": "EMPTY_PLOTS", "message": "plots 列表为空，K线图上将无任何内容显示"})
    ...
    for m in re.finditer(r"['\"]name['\"]\s*:\s*", code):
        ...
        if end_quote == -1:
            end_quote = code.find('"', start)
        if end_quote == -1:
            hints.append({"severity": "error", "code": "INVALID_PLOT_NAME", "message": "plot 的 name 值格式错误"})
...
def _check_signals_structure(code: str) -> List[Dict[str, Any]]:
    ...
    if has_signals_key and re.search(r"['\"]signals['\"]\s*:\s*\[\s*\]", code):
        hints.append({"severity": "info", "code": "EMPTY_SIGNALS", "message": "signals 列表为空，不会有买卖信号标记"})
...
def _check_nan_handling(code: str) -> List[Dict[str, Any]]:
    ...
    has_shift = bool(re.search(r'\.shift\s*\(', c))
    has_mean = bool(re.search(r'\.mean\s*\(', c))

    if has_rolling or has_ewm:
        if not re.search(r'\.fillna\s*\(', c) and not re.search(r'\.dropna\s*\(', c):
            hints.append({
                "severity": "warn",
    ...
    if re.search(r'output\s*=\s*\{[^}]*["\']data["\']', c):
        if not re.search(r'\.tolist\s*\(\s*\)', c[:500]):
            hints.append({
                "severity": "info",
...
def _check_import_safety(code: str) -> List[Dict[str, Any]]:
    ...
    for mod, reason in dangerous_imports:
        ...
        if re.search(pattern, c, re.MULTILINE):
            hints.append({"severity": "error", "code": f"DANGEROUS_IMPORT_{mod.upper()}", "message": reason})

    dangerous_funcs = ['eval(', 'exec(', 'compile(', '__import__(']
    for func in dangerous_funcs:
        if func in c:
            hints.append({"severity": "error", "code": "DANGEROUS_FUNC", "message": f"使用了危险函数 {func.strip('(')}"})
...
def analyze_indicator_code_quality(code: str) -> List[Dict[str, Any]]:
    ...
    if len(raw) > 10000:
        hints.append({"severity": "warn", "code": "CODE_TOO_LONG", "message": f"代码过长({len(raw)}字符)，可能影响执行性能"})

    name_ok, desc_ok = _has_my_indicator_meta(raw)
    if not name_ok:
        hints.append({"severity": "warn", "code": "MISSING_INDICATOR_NAME", "message": "缺少 my_indicator_name 变量声明"})
    if not desc_ok:
        hints.append({"severity": "info", "code": "MISSING_INDICATOR_DESCRIPTION", "message": "缺少 my_indicator_description 变量声明"})

    if not _has_df_copy(raw):
        hints.append({"severity": "info", "code": "MISSING_DF_COPY", "message": "建议在开头添加 df = df.copy() 以避免修改原始数据"})

    if not _has_output_dict(raw):
        hints.append({"severity": "error", "code": "MISSING_OUTPUT", "message": "缺少 output 字典变量定义"})
    else:
    ...
    if not re.search(r'pd\.|numpy|np\.', raw):
        hints.append({"severity": "info", "code": "NO_PANDAS_USAGE", "message": "未检测到 pandas/numpy 使用，指标计算通常需要这些库"})
