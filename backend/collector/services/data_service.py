...
class DataService:
    ...
    @staticmethod
    def get_existing_trading_pairs() -> List[Dict[str, Any]]:
        """获取数据库中现有的交易对和时间间隔
        
        Returns:
            List[Dict[str, Any]]: 交易对和时间间隔的列表
        """
        from ..db.database import SessionLocal, init_database_config
        from ..db.models import CryptoSpotKline, CryptoFutureKline
        from sqlalchemy import func
        
        logger.info("开始获取数据库中现有的交易对和时间间隔")
        ...
        try:
            ...
            # 查询现货K线交易对
            spot_pairs = db.query(
                CryptoSpotKline.symbol,
                CryptoSpotKline.interval,
                func.max(CryptoSpotKline.date).label('last_date')
            ).group_by(CryptoSpotKline.symbol, CryptoSpotKline.interval).all()
            
            for pair in spot_pairs:
                result.append({
                    'symbol': pair.symbol,
                    'interval': pair.interval,
                    'last_date': pair.last_date,
                    'candle_type': 'spot'
                })
            
            # 查询合约K线交易对
            future_pairs = db.query(
                CryptoFutureKline.symbol,
                CryptoFutureKline.interval,
                func.max(CryptoFutureKline.date).label('last_date')
            ).group_by(CryptoFutureKline.symbol, CryptoFutureKline.interval).all()
            ...
            logger.info(f"成功获取到 {len(result)} 个交易对和时间间隔组合")
            return result
        except Exception as e:
            logger.error(f"获取交易对和时间间隔失败: {e}")
            logger.exception(e)
            return []
        finally:
            db.close()
    ...
    @staticmethod
    def sync_single_pair(symbol: str, interval: str, start_time: datetime, end_time: datetime, candle_type: str, exchange: str = "binance") -> Dict[str, Any]:
        """同步单个交易对的K线数据
        ...
        Args:
            ...
            start_time: 开始时间
            end_time: 结束时间
            candle_type: 蜡烛类型（spot/future）
            exchange: 交易所，默认binance
            
        Returns:
            Dict[str, Any]: 同步结果
        """
        logger.info(f"开始同步单个交易对数据: symbol={symbol}, interval={interval}, start_time={start_time}, end_time={end_time}, candle_type={candle_type}")
        ...
        try:
            ...
            def progress_callback(current, completed, total, failed, status=None):
                """进度回调函数"""
                logger.debug(f"同步进度: 当前={current}, 已完成={completed}, 总任务={total}, 失败={failed}, 状态={status}")
            
            # 调用crypto方法下载数据
            get_data.crypto(
                exchange=exchange,
                save_dir=None,  # 不保存到文件，只写入数据库
            ...
            logger.info(f"成功同步交易对数据: symbol={symbol}, interval={interval}, candle_type={candle_type}")
            return {
                'success': True,
                'symbol': symbol,
                'interval': interval,
                'candle_type': candle_type,
                'message': '同步成功',
                'data_volume': 0  # 暂时无法获取具体数据量，后续可以改进
            }
        except Exception as e:
            logger.error(f"CCXT API同步失败，尝试使用CSV下载: {e}")
            
            try:
                # 回退方案：使用CSV下载和数据库导入
                # 这里可以根据实际情况实现CSV下载逻辑
                logger.error(f"CSV下载功能尚未实现，交易对 {symbol} 同步失败")
                return {
                    'success': False,
                    'symbol': symbol,
                    'interval': interval,
                ...
                logger.error(f"CSV下载同步失败: {csv_error}")
                return {
                    'success': False,
                    'symbol': symbol,
                    'interval': interval,
                    'candle_type': candle_type,
                    'message': f"同步失败: {str(csv_error)}",
                    'data_volume': 0
                }
    
    @staticmethod
    def sync_incremental_kline_data():
        """增量同步K线数据
        
        每30分钟执行一次，查询数据库中现有的交易对，
        为每个交易对确定增量同步的时间范围，
        使用CCXT API或CSV下载进行数据同步
        """
        logger.info("开始执行增量K线数据同步任务")
        
        # 获取当前时间
        end_time = datetime.now()
        
        # 获取数据库中现有的交易对和时间间隔
        trading_pairs = DataService.get_existing_trading_pairs()
        ...
        for pair in trading_pairs:
            try:
                ...
                interval = pair['interval']
                last_date = pair['last_date']
                candle_type = pair['candle_type']
                
                # 确定开始时间
                start_time_pair = last_date
                
                logger.info(f"准备同步: symbol={symbol}, interval={interval}, last_date={last_date}, candle_type={candle_type}")
                
                # 执行同步
                result = DataService.sync_single_pair(
                    symbol=symbol,
                    interval=interval,
                    start_time=start_time_pair,
                    end_time=end_time,
                    candle_type=candle_type
                )
                ...
                if result['success']:
                    success_count += 1
                    logger.info(f"交易对同步成功: {symbol} - {interval}")
                else:
                    failed_count += 1
                    logger.error(f"交易对同步失败: {symbol} - {interval}, 原因: {result['message']}")
            except Exception as e:
                failed_count += 1
                logger.error(f"处理交易对时发生异常: {e}")
                logger.exception(e)
                continue
        ...
        # 记录任务执行结果
        logger.info(f"增量K线数据同步任务执行完成")
        logger.info(f"统计信息: 总交易对={total_pairs}, 成功={success_count}, 失败={failed_count}")
        logger.info(f"执行时间: {execution_time.total_seconds():.2f}秒")
        
        # 如果有失败的交易对，记录警告
        if failed_count > 0:
            logger.warning(f"有 {failed_count} 个交易对同步失败，占总数的 {failed_count/total_pairs*100:.1f}%")
