...
# 数据库连接和模型
from collector.db.models import CryptoFutureKline, CryptoSpotKline, StockKline
...
class KlineHealthChecker:
    ...
    def check_validity(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if not abnormal_price_changes.empty:
            ...
            result["abnormal_price_changes"] = [
                {"timestamp": str(idx), "change_pct": round(row["price_change_pct"], 2)}
        ...
        if not price_gaps.empty:
            result["status"] = "fail"
            result["price_gaps"] = [
                {"timestamp": str(idx), "gap_pct": round(row["open_gap_pct"], 2)} for idx, row in price_gaps.iterrows()
            ]
    ...
    def check_consistency(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        try:
            ...
            result["status"] = "fail"
            result["time_format_issues"].append(f"时间格式错误: {e!s}")
        ...
        if "adj_factor" in df.columns:
            ...
            if len(adj_factor_diff.dropna()) > 0:
                ...
                # 如果混合情况较多，说明复权方式可能不一致
                    ...
                    result["status"] = "fail"
                    result["inconsistent_adj_factors"].append(
                        f"复权因子变化不一致: 递增 {increasing} 条，递减 {decreasing} 条"
                    )
    ...
    def check_uniqueness(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if duplicate_timestamps.any():
            ...
            # 获取所有重复的时间戳值
            duplicate_dates = df[duplicate_timestamps]["timestamp"].unique() if "timestamp" in df.columns else []
    ...
    def check_logic(self, df: pd.DataFrame, interval: str) -> dict[str, Any]:
        ...
        if not potential_suspension.empty:
            # 检查停牌期间的价格字段是否完整
            suspension_missing_prices = potential_suspension[
                (potential_suspension["open"].isnull())
            ...
            if not suspension_missing_prices.empty:
                result["status"] = "fail"
                result["suspension_issues"].append(f"停牌期间价格字段缺失: {len(suspension_missing_prices)} 条记录")
        ...
        if not price_limit_issues.empty:
            result["status"] = "fail"
            result["price_limit_issues"].append(f"发现 {len(price_limit_issues)} 条记录超出涨跌停限制")
