...
class BaseProductListFetcher(ProductListFetcher):
    ...
    def _fetch_from_database(
        ...
        """从数据库获取商品列表的通用方法"""
        logger.info(
            f"从数据库获取商品列表: table={table_name}, exchange={exchange}, filter={filter}, limit={limit}, offset={offset}"
        )
        ...
        if table_name == "crypto_symbol":
            ...
            # 默认返回空列表
            return {"success": True, "message": "查询商品列表成功", "products": []}
...
class StockProductListFetcher(BaseProductListFetcher):
    ...
    def fetch_products(
        ...
        if not db:
            return {"success": False, "message": "数据库会话未初始化", "products": []}
...
class ProductListFactory:
    ...
    @staticmethod
    def create_fetcher(market_type: str, crypto_type: str | None = None) -> ProductListFetcher:
        """创建商品列表获取器
