...
class ScheduledTaskManager:
    ...
    def add_task(self, task_info: dict):
        ...
        try:
            ...
            ScheduledTaskBusiness.update(task_id=task_id, next_run_time=next_run_time)
    ...
    def pause_task(self, task_id: int):
        ...
        try:
            ...
            # 更新数据库中的任务状态
            ScheduledTaskBusiness.update(task_id=task_id, status="paused")
    ...
    def resume_task(self, task_id: int):
        ...
        try:
            ...
            # 更新数据库中的任务状态
            ScheduledTaskBusiness.update(task_id=task_id, status="running")

            logger.info(f"定时任务已恢复: task_id={task_id}")
        except Exception as e:
            logger.error(f"恢复定时任务失败: task_id={task_id}, error={e}")

    def get_job(self, task_id: int) -> Job | None:
        """获取任务
    ...
    def _download_crypto_data(self, task_info: dict):
        ...
        try:
            ...
            logger.info(
                f"开始下载加密货币数据: symbols={symbols}, exchange={exchange}, incremental_enabled={incremental_enabled}, last_collected_date={last_collected_date}"
            )
