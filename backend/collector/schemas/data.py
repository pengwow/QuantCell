...
class DownloadCryptoRequest(BaseModel):
    ...
    save_dir: str | None = Field(None, description="保存目录（内部参数，忽略）")
    mode: str = Field(default="inc", description="下载模式，可选'inc'（增量）或'full'（全量）")
    data_type: str = Field(
        default="kline",
        description="数据类型：kline/aggTrades/trades/bookDepth/bookTicker/markPriceKlines/indexPriceKlines/premiumIndexKlines/fundingRate/openInterest",
    )
    ...
    def validate_data_type_market(self):
        if self.data_type not in VALID_DATA_TYPES:
            msg = f"无效的 data_type: {self.data_type}，可选值：{', '.join(sorted(VALID_DATA_TYPES))}"
            raise ValueError(msg)
        if self.market not in VALID_MARKETS:
            msg = f"无效的 market: {self.market}，可选值：{', '.join(sorted(VALID_MARKETS))}"
            raise ValueError(msg)
        if self.market == "spot" and self.data_type in _FUTURES_ONLY_TYPES:
            msg = f"data_type '{self.data_type}' 仅支持 market=um 或 cm"
            raise ValueError(msg)
        # K 线类必须传 interval
        if self.data_type in _KLINE_TYPES and not self.interval:
            msg = f"data_type '{self.data_type}' 必须指定 interval (例如 ['15m'])"
            raise ValueError(msg)
        return self
...
class TaskStatusResponse(BaseModel):
    ...
    current: str | None = Field(None, description="当前处理的项目")
...
class ExportCryptoRequest(BaseModel):
    ...
    exchange: str = Field(default="binance", description="交易所")
    candle_type: str = Field(default="spot", description="蜡烛图类型")
    save_dir: str | None = Field(None, description="保存目录，如果不提供则使用默认值")
    max_workers: int = Field(default=1, description="最大工作线程数")
