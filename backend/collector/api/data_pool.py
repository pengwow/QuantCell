...
def get_all_pools(
    ...
    try:
        ...
        return ApiResponse(code=0, message="获取所有数据池成功", data=pools)
...
def get_pool(
    ...
    try:
        ...
        if pool:
            ...
            return ApiResponse(code=0, message="获取数据池成功", data=pool)
        else:
            logger.warning(f"数据池不存在: {pool_id}")
            return ApiResponse(code=1, message="数据池不存在", data={"pool_id": pool_id})
    except Exception as e:
    ...
    try:
        ...
        if pool_id:
            ...
            return ApiResponse(
                code=0,
                message="创建数据池成功",
                data=DataPoolCreateResponse(pool_id=pool_id, name=pool.name, type=pool.type),
            )
        else:
...
def get_pool_assets(
    ...
    try:
        ...
        return ApiResponse(
            code=0,
            message="获取数据池资产成功",
            data=DataPoolAssetsResponse(pool_id=pool_id, assets=assets),
        )
    except Exception as e:
...
def add_pool_assets(
    ...
    try:
        logger.info(
            f"开始向数据池添加资产: pool_id={pool_id}, asset_count={len(assets.assets)}, asset_type={assets.asset_type}"
        )
        ...
        if success:
            ...
            return ApiResponse(
                code=0,
                message="向数据池添加资产成功",
                data=DataPoolAssetAddResponse(pool_id=pool_id, added_count=len(assets.assets)),
            )
        else:
...
def get_collection_symbols(
    ...
    try:
        ...
        for pool in asset_pools:
            ...
            pool_data.append({"id": pool["id"], "name": pool["name"], "symbols": assets})
        ...
        return ApiResponse(
            code=0,
            message="获取品种选项数据成功",
            data=CollectionSymbolsResponse(data_pools=pool_data, direct_symbols=direct_symbols),
        )
    except Exception as e:
    ...
    try:
        ...
        if success:
            ...
            return ApiResponse(
                code=0,
                message="从数据池移除资产成功",
                data=DataPoolAssetRemoveResponse(pool_id=pool_id, removed_count=len(assets.assets)),
            )
        else:
