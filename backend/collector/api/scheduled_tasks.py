...
async def get_all_scheduled_tasks(
    ...
    try:
        ...
        return ApiResponse(
            code=0,
            message="获取定时任务成功",
            data={"tasks": list(tasks.values()), "total": len(tasks)},
        )
...
async def get_scheduled_task(task_id: int):
    ...
    try:
        ...
        if not task:
            ...
            return ApiResponse(code=404, message="定时任务不存在", data={})

        logger.info(f"获取定时任务详情成功: task_id={task_id}")

        return ApiResponse(code=0, message="获取定时任务详情成功", data={"task": task})
    except Exception as e:
...
async def create_scheduled_task(request: ScheduledTaskCreate):
    ...
    try:
        ...
        if not task_id:
            ...
            return ApiResponse(code=500, message="创建定时任务失败", data={})
        ...
        return ApiResponse(code=0, message="创建定时任务成功", data={"task": task})
    except Exception as e:
...
async def update_scheduled_task(task_id: int, request: ScheduledTaskUpdate):
    ...
    try:
        ...
        if not success:
            ...
            return ApiResponse(code=500, message="更新定时任务失败", data={})
        ...
        return ApiResponse(code=0, message="更新定时任务成功", data={"task": task})
    except Exception as e:
...
async def delete_scheduled_task(task_id: int, current_user: dict = Depends(get_current_user)):
    """删除定时任务
    ...
    try:
        ...
        if not success:
            ...
            return ApiResponse(code=500, message="删除定时任务失败", data={})

        logger.info(f"删除定时任务成功: task_id={task_id}")

        return ApiResponse(code=0, message="删除定时任务成功", data={})
    except Exception as e:
...
async def run_scheduled_task_now(task_id: int):
    ...
    try:
        ...
        return ApiResponse(code=0, message="定时任务已开始执行", data={})
    except Exception as e:
        logger.error(f"立即执行定时任务失败: task_id={task_id}, error={e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def pause_scheduled_task(task_id: int):
    ...
    try:
        ...
        return ApiResponse(code=0, message="暂停定时任务成功", data={})
    except Exception as e:
...
async def resume_scheduled_task(task_id: int):
    ...
    try:
        ...
        return ApiResponse(code=0, message="恢复定时任务成功", data={})
    except Exception as e:
