...
async def get_symbols(
    ...
    try:
        ...
        # 从数据库获取
        result = await market_data_service.get_symbols_from_db(exchange=exchange, limit=limit, offset=offset)

        return ApiResponse(code=0, message="获取货币对列表成功", data=result)
    except Exception as e:
        return ApiResponse(code=500, message=f"获取货币对列表失败: {e!s}", data=None)
...
async def get_market_data(request: MarketDataRequest):
    ...
    try:
        if not request.symbols:
            return ApiResponse(code=400, message="symbols列表不能为空", data=None)

        # 限制每次请求的最大数量
        if len(request.symbols) > 100:
            return ApiResponse(code=400, message="每次最多查询100个货币对", data=None)
        ...
        return ApiResponse(code=0, message="获取市场数据成功", data=result)
    except Exception as e:
        return ApiResponse(code=500, message=f"获取市场数据失败: {e!s}", data=None)
...
async def sync_symbols(exchange: str = "binance"):
    ...
    try:
        ...
        return ApiResponse(code=500, message=f"同步货币对失败: {e!s}", data=None)
