...
class SystemLog(TimezoneAwareBase):
    """系统日志SQLAlchemy模型

    对应system_logs表的SQLAlchemy模型定义，用于存储结构化日志数据
    支持多种日志级别和类型，便于查询和分析
    """
    __tablename__ = "system_logs"

    id = Column(Integer, primary_key=True, autoincrement=True, index=True)
    timestamp = Column(DateTime(timezone=True), nullable=False, index=True)  # 日志时间戳
    ...
    module = Column(String(200), nullable=True, index=True)  # 模块名称
    function = Column(String(200), nullable=True)  # 函数名称
    line = Column(Integer, nullable=True)  # 代码行号
    logger_name = Column(String(200), nullable=True, index=True)  # 日志器名称
    log_type = Column(String(50), nullable=False, index=True)  # 日志类型：system, application, strategy, backtest, trade, api, database, exception
    extra_data = Column(Text, nullable=True)  # 额外数据，JSON格式
    exception_info = Column(Text, nullable=True)  # 异常堆栈信息
    trace_id = Column(String(100), nullable=True, index=True)  # 跟踪ID，用于关联同一请求的日志
    created_at = Column(DateTime(timezone=True), server_default=func.now(), index=True)  # 记录创建时间

    # 添加索引优化查询性能
    __table_args__ = (
        sqlalchemy.Index('idx_system_logs_level_timestamp', 'level', 'timestamp'),
        sqlalchemy.Index('idx_system_logs_type_timestamp', 'log_type', 'timestamp'),
        sqlalchemy.Index('idx_system_logs_trace_id', 'trace_id'),
        sqlalchemy.Index('idx_system_logs_module_timestamp', 'module', 'timestamp'),
    )


class SystemLogBusiness:
    """系统日志业务类

    用于操作system_logs表，提供CRUD操作方法
    """
    ...
    def create(
        ...
        try:
            log_entry = SystemLog(
                ...
                function=function[:200] if function else None,
                line=line,
                logger_name=logger_name[:200] if logger_name else None,
                log_type=log_type,
                extra_data=json.dumps(extra_data, ensure_ascii=False) if extra_data else None,
                exception_info=exception_info[:4000] if exception_info else None,
            ...
            db.rollback()
            print(f"[SystemLogBusiness] 创建日志记录失败: {e}")
            return False
        finally:
            db.close()
    ...
    def get_by_id(log_id: int) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Optional[Dict[str, Any]]: 日志记录字典，不存在返回None
        """
        from .database import SessionLocal, init_database_config
        init_database_config()
        db: Session = SessionLocal()
        try:
            log = db.query(SystemLog).filter_by(id=log_id).first()
            if not log:
                return None
            return {
                "id": log.id,
            ...
            print(f"[SystemLogBusiness] 获取日志记录失败: {e}")
            return None
        finally:
            db.close()
    ...
    def query_logs(
        ...
        Args:
            ...
            start_time: 开始时间
            end_time: 结束时间
            keyword: 关键词搜索（匹配message）
            page: 页码
            page_size: 每页数量

        Returns:
            Dict[str, Any]: 包含日志列表和分页信息
        """
        from .database import SessionLocal, init_database_config
        init_database_config()
        db: Session = SessionLocal()
        try:
            ...
            print(f"[SystemLogBusiness] 查询日志失败: {e}")
            return {
                "logs": [],
                "pagination": {
                    "page": page,
    ...
    def delete_old_logs(days: int = 30) -> int:
        ...
        try:
            ...
            result = db.query(SystemLog).filter(SystemLog.timestamp < cutoff_date).delete()
            db.commit()
            return result
        except Exception as e:
            db.rollback()
            print(f"[SystemLogBusiness] 删除旧日志失败: {e}")
            return 0
        finally:
            db.close()
    ...
    def get_log_statistics(
        ...
        try:
            ...
            return {
                ...
                "by_type": {log_type: count for log_type, count in type_stats},
            }
        except Exception as e:
            print(f"[SystemLogBusiness] 获取日志统计失败: {e}")
            return {
                "total_count": 0,
                "by_level": {},
                "by_type": {},
