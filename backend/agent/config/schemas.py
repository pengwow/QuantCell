...
class ParamTemplateItem(BaseModel):
    ...
    env_key: str | None = Field(default=None, description="对应的环境变量名")
    description: str = Field(default="", description="参数描述")
    validation: dict[str, Any] | None = Field(default=None, description="验证规则 (如 {'min': 1, 'max': 10})")
...
class ToolParamTemplate(BaseModel):
    ...
    params: dict[str, ParamTemplateItem] = Field(default_factory=dict, description="参数字典")
...
class ParamValueInfo(BaseModel):
    ...
    source: str = Field(..., description="值的来源: database | environment | default")
    sensitive: bool = Field(default=False, description="是否敏感参数")
    type: str | None = Field(default=None, description="参数类型")
    description: str = Field(default="", description="参数描述")
...
class ToolParamsResponse(BaseModel):
    ...
    params: dict[str, ParamValueInfo] = Field(default_factory=dict, description="参数详情")
...
class ToolListItem(BaseModel):
    ...
    has_required_params: bool = Field(default=False, description="是否所有必需参数都已配置")
...
class ExportConfigResponse(BaseModel):
    ...
    tools: dict[str, dict[str, Any]] = Field(default_factory=dict, description="工具配置字典")
