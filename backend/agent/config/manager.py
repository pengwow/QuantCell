...
def mask_sensitive_value(value: str | None, show_chars: int = 4) -> str:
    """
    脱敏处理
...
class ToolParamManager:
    ...
    @staticmethod
    def get_tool_params(tool_name: str, include_sensitive: bool = False) -> dict[str, dict[str, Any]]:
        """
        获取工具的所有参数
    ...
    @staticmethod
    def set_tool_param(tool_name: str, param_name: str, value: Any) -> bool:
        """
        设置工具参数
        ...
        if not template:
            msg = f"未知参数: {tool_name}.{param_name}"
            raise ValueError(msg)

        is_valid, error_msg = ToolParamResolver.validate(tool_name, param_name, value)
        if not is_valid:
    ...
    @staticmethod
    def batch_update(tool_name: str, params: dict[str, Any], overwrite: bool = False) -> dict[str, list[str]]:
        """
        批量更新参数
        ...
        """
        result: dict[str, list[str]] = {"updated": [], "skipped": [], "errors": []}
        ...
        for param_name, value in params.items():
            try:
                ...
                result["errors"].append(f"{param_name}: {e!s}")
            except Exception as e:
                result["errors"].append(f"{param_name}: {e!s}")
    ...
    @staticmethod
    def export_config(tool_name: str | None = None) -> dict[str, Any]:
        """
        导出配置（敏感值始终脱敏）
        ...
        for tname in tools_to_export:
            try:
                params = ToolParamManager.get_tool_params(tname, include_sensitive=False)

                export_data["tools"][tname] = {pname: pinfo["value"] for pname, pinfo in params.items()}
            except ValueError as e:
    ...
    @staticmethod
    def import_config(config: dict[str, Any], overwrite: bool = False) -> tuple[int, int, list[str]]:
        """
        导入配置
        ...
        for tool_name, params in tools_config.items():
            ...
            result = ToolParamManager.batch_update(tool_name, params, overwrite)
...
class ToolParamValidator:
    ...
    @staticmethod
    def validate(tool_name: str, param_name: str, value: Any) -> tuple[bool, str]:
        """
        验证参数值
