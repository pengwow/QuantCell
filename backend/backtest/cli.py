...
def run(
    ...
    engine: Annotated[EngineType, typer.Option("--engine", "-e", help="回测引擎类型（default/event）")] = EngineType.EVENT,
    ...
    # 设置日志
    logger = get_logger(__name__, LogType.BACKTEST)
    
    logger.info("=" * 70)
    logger.info("开始回测")
    logger.info(f"引擎类型: {engine.value}")
    logger.info("=" * 70)
    
    # 验证参数
    if time_range and not validate_time_range(time_range):
        logger.error(f"错误：时间范围格式错误: {time_range}")
        logger.error("支持的格式：")
        logger.error("  - YYYYMMDD-YYYYMMDD (例如：20240101-20241231)")
        logger.error("  - ISO格式 (例如：2024-01-01 00:00:00-2024-12-31 23:59:59)")
        raise typer.Exit(1)
    
    if symbols and not validate_symbols(symbols):
        logger.error(f"错误：货币对格式错误")
        raise typer.Exit(1)
    
    if timeframes and not validate_timeframes(timeframes):
        logger.error(f"错误：时间周期格式错误: {timeframes}")
        logger.error("支持的周期：15m, 30m, 1h, 4h, 1d")
        raise typer.Exit(1)
    
    if trading_mode and not validate_trading_mode(trading_mode):
        logger.error(f"错误：交易模式错误: {trading_mode}")
        logger.error("支持的模式：spot（现货）、futures（合约）、perpetual（永续合约）")
        raise typer.Exit(1)
    
    # 检查是否同时指定了 symbols 和 pool
    if symbols and pool:
        logger.error("错误：不能同时指定 --symbols 和 --pool 参数")
        raise typer.Exit(1)
...
def _run_default_backtest(
    ...
    if pool:
        ...
        try:
            ...
            if not symbols_list:
                logger.error(f"错误：自选组合 '{pool}' 中没有货币对")
                raise typer.Exit(1)
            logger.info(f"成功获取 {len(symbols_list)} 个货币对: {', '.join(symbols_list)}")
        ...
        try:
            ...
            if incomplete_results:
                ...
                for result in incomplete_results:
                    ...
                    logger.info(f"  - {result.symbol} {result.timeframe}: 覆盖率 {result.coverage_percent:.1f}%，缺失 {missing_pct:.1f}%")
                    if result.warnings:
                        for warning in result.warnings:
                            logger.info(f"    • {warning}")
                if ignore_missing:
                    logger.info(f"  已根据 --ignore-missing 参数继续回测")
    ...
    logger.info(f"时间周期: {', '.join(timeframes_list)}")
...
def _run_event_backtest(
    ...
    try:
        ...
        logger.error(f"错误：无法导入事件驱动引擎模块: {e}")
    ...
    logger.info(f"时间周期: {', '.join(timeframes_list)}")
    logger.info(f"交易模式: {trading_mode}")
    logger.info(f"初始资金: {init_cash} {base_currency}")
    logger.info(f"手续费率: {fees}")
    ...
    try:
        ...
        # 创建第一个品种以获取venue
        logger.info(f"创建交易品种: {first_symbol}")
        try:
            ...
            logger.info(f"交易品种所属交易所: {instrument_venue}")
        except Exception as e:
        ...
        # 添加交易所（使用品种对应的venue）
        logger.info(f"添加交易所: {instrument_venue}")
        from decimal import Decimal
        engine.add_venue(
        ...
        for symbol in symbols_list:
            ...
            if key not in data_dict:
                logger.info(f"警告：跳过 {key}，数据未加载")
                continue

            df = data_dict[key]

            # 创建交易品种
            logger.info(f"创建交易品种: {symbol}")
            try:
        ...
        logger.info(f"加载事件驱动策略: {strategy_name}")
        strategy = _load_event_strategy_multi(
            strategy_name, strategy_params, bar_types, instruments
        )

        if strategy is None:
            logger.error(f"错误：无法加载策略 {strategy_name}")
            logger.info("请确保策略继承自EventDrivenStrategy或使用兼容的策略基类")
            raise typer.Exit(1)
        ...
        logger.info(f"\n投资组合回测结果摘要:")
        logger.info(f"  品种数量: {len(symbols_list)}")
        logger.info(f"  总收益率: {portfolio_metrics.get('total_return', 0):.2f}%")
        logger.info(f"  总交易次数: {portfolio_metrics.get('total_trades', 0)}")
        logger.info(f"  胜率: {portfolio_metrics.get('win_rate', 0):.2f}%")
        logger.info(f"  盈亏比: {portfolio_metrics.get('profit_factor', 0):.2f}")
        logger.info(f"  夏普比率: {portfolio_metrics.get('sharpe_ratio', 0):.4f}")
        logger.info(f"  最大回撤: {portfolio_metrics.get('max_drawdown', 0):.2f}%")
        logger.info(f"  总盈亏: {portfolio_metrics.get('total_pnl', 0):.2f}")
        logger.info("=" * 70)
        ...
        logger.error(f"错误：事件驱动回测执行失败: {e}")
        import traceback
        traceback.print_exc()
        raise typer.Exit(1)
...
def _load_event_strategy_multi(
    ...
    try:
        ...
        if is_core_strategy:
            ...
            logger.info(f"成功加载策略: {strategy_class.__name__}（支持 {len(instruments)} 个品种）")
        else:
...
def plot(
    ...
    if not input_path.exists() and not input_path.is_absolute():
        ...
        if full_path.exists():
            ...
            logger.info(f"✓ 在默认目录找到文件: {input_path}")

    # 验证文件是否存在
    if not input_path.exists():
        logger.error(f"错误：文件不存在: {input_file}")
        logger.info(f"  查找路径: {input_path}")
        if not input_path.is_absolute():
            default_results_dir = Path(__file__).parent / 'results'
            logger.info(f"  默认结果目录: {default_results_dir}")
        raise typer.Exit(1)

    if not str(input_path).lower().endswith('.json'):
        logger.error("错误：仅支持JSON格式文件")
        raise typer.Exit(1)
    ...
    logger.info(f"✓ 成功加载回测结果文件: {input_file}")
    logger.info(f"  包含 {symbol_count} 个交易对的数据")
    print()
    ...
    try:
        ...
        if merged and symbol_count > 1:
            logger.info(f"\n生成的报告文件:")
            logger.info(f"  • 合并报告: {merged_report_path}")
            if separate:
                logger.info(f"  • 单独图表: {separate_output}")
        
    except Exception as e:
        logger.error(f"错误：绘制图表失败: {e}")
        import traceback
        traceback.print_exc()
        raise typer.Exit(1)
