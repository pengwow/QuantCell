...
class BacktestDataDownloader:
    ...
    def _download_missing_data(
        ...
        logger.info(
            f"增量下载完成: {symbol} {interval}, 成功率 {success_rate * 100:.1f}% ({total_success}/{len(missing_ranges)})"
        )
    ...
    def _download_with_standalone(
        ...
        try:
            ...
            if success:
                progress.update(status="completed", progress=100.0, message="下载完成")
                self._notify_progress(progress)
                logger.info(f"独立下载完成: {symbol} {interval}")
            else:
                progress.update(status="failed", message="下载失败")
                self._notify_progress(progress)
    ...
    def _wait_for_download(self, task_id: str, symbol: str, interval: str, max_wait_time: int) -> bool:
        """
        等待下载完成
        ...
        while time.time() - start_time < max_wait_time:
            try:
                ...
                if status == "completed":
                    ...
                    progress.update(status="failed", message="下载失败", error=error_msg)
                    self._notify_progress(progress)
    ...
    def ensure_multi_symbol_data_complete(
        ...
        for symbol in symbols:
            ...
            if progress_callback:
                ...
                def symbol_callback(p, s=symbol):
                    return progress_callback(s, p)
