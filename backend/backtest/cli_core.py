...
from backtest.progress import ConsoleProgressBar
from utils.time_parser import datetime_to_timestamp, parse_time_range
...
class DataDownloadResult:
    ...
    timeframe: str
    success: bool
    failure_type: DownloadFailureType | None = None
...
class CLICore:
                    ...
                    try:
                        ...
                        if start_date and end_date:
                            ...
                            if not integrity_result.is_complete:
                                missing_percentage = (
                                    100.0 - integrity_result.coverage_percent
                                    if hasattr(integrity_result, "coverage_percent")
                                    else 0
                                )
                                ...
                                if ignore_missing:
                                    if show_progress and progress_bar:
                                        progress_bar.set_message(
                                            f"⚠️  {key} 数据不完整 ({missing_percentage:.1f}%)，已忽略"
                                        )
                                    logger.warning(
                                        f"  ⚠️ {key} 数据不完整 ({missing_percentage:.1f}%)，根据 --ignore-missing 参数继续"
                                    )
                                # 如果允许自动下载，尝试下载缺失数据
                                elif auto_download:
                                    if show_progress and progress_bar:
                                        progress_bar.set_message(
                                            f"⬇️  下载 {key} 缺失数据 ({missing_percentage:.1f}%)..."
                                        )
                                    else:
    ...
    def _load_klines_from_db(
        ...
        try:
            ...
            # 添加诊断日志
            logger.info(
                f"[_load_klines_from_db] 查询参数: symbol={symbol}, normalized={normalized_symbol}, interval={timeframe}"
            )
    ...
    def _determine_failure_type(self, symbol: str, trading_mode: str = "spot") -> DownloadFailureType:
        """
        判断下载失败类型
    ...
    def load_strategy(self, strategy_name: str, strategy_params: dict[str, Any]) -> Any:
        """
        加载策略
    ...
    def save_to_database(self, results: dict[str, Any], strategy_name: str, config: dict[str, Any]) -> bool:
        """
        保存回测结果到数据库
        ...
        try:
            ...
            try:
                ...
                if "portfolio" in results:
                    ...
                    # 获取所有交易对
                    symbols = [k for k in results if k != "portfolio"]
...
def get_system_config() -> dict[str, Any]:
    ...
    try:
        ...
        # 读取时间周期默认值
        timeframes_config = conn.execute("SELECT value FROM system_config WHERE key = 'default_timeframes'").fetchone()
        ...
        return {"default_trading_mode": "spot", "default_timeframes": ["1h"]}
...
def get_symbols_from_data_pool(pool_name: str) -> list[str]:
    ...
    try:
        ...
        msg = f"从自选组合获取货币对失败: {e}"
        raise ValueError(msg)


def generate_test_data(n_steps: int = 1000, base_price: float = 50000.0, volatility: float = 0.001) -> pd.DataFrame:
    """
    生成测试数据
