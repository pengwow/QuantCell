...
def _convert_equity_curve(advanced_result: Any) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for _, row in df_copy.iterrows():
            ...
            # 添加其他可能的字段
            for col in df_copy.columns:
                if col not in record and col not in [
                    "datetime",
                    "equity",
                    "Equity",
                    "balance",
...
def _convert_single_trade(
    trade_data: pd.Series | dict[str, Any],
) -> dict[str, Any] | None:
    """
    转换单条交易记录
    ...
    try:
        # 统一转换为字典
        trade_dict = trade_data.to_dict() if isinstance(trade_data, pd.Series) else dict(trade_data)
