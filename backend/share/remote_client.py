...
class RemoteShareClient:
    ...
    def __init__(self, config: ShareRemoteConfig | None = None) -> None:
    ...
    async def upload(self, snapshot: dict[str, Any], token_hash: str, worker_id: int) -> dict[str, Any]:
        ...
        if not self.config.is_ready:
            msg = "远端未就绪：share_remote 未启用或缺少 api_key/hmac_secret"
            raise RemoteShareError(msg)
        ...
        if not remote_id or not short_url:
            msg = f"远端响应缺少 id/short_url: {resp}"
            raise RemoteShareError(msg)
            ...
            async with session.post(url, json=body, headers=headers) as resp:
                ...
                if 200 <= resp.status < 300:
                    try:
                        ...
                        msg = f"远端 auto-register 返回非 JSON (status={resp.status}): {e}"
                        raise RemoteShareError(msg)
                msg = f"auto-register 失败 status={resp.status} body={text[:500]}"
                raise RemoteShareError(msg)
                ...
                async with (
                    ...
                    if 200 <= resp.status < 300:
                        ...
                        try:
                            ...
                            msg = f"远端返回非 JSON (status={resp.status}): {e}"
                            raise RemoteShareError(msg)
                    # 4xx 不重试（业务错误）
                    if 400 <= resp.status < 500:
                        msg = f"远端拒绝请求 status={resp.status} body={text[:500]}"
                        raise RemoteShareError(msg)
                    # 5xx 可重试
                    last_err = RemoteShareError(f"远端 5xx status={resp.status} body={text[:200]}")
                    logger.warning(f"远端 5xx，尝试重试 {attempt}/{self.config.max_retries} url={url}")
            except RemoteShareError:
                # 业务错误：直接抛出
                raise
            except Exception as e:  # 网络错误：可重试
                last_err = e
                logger.warning(f"远端调用网络错误，尝试重试 {attempt}/{self.config.max_retries} url={url} err={e}")

            if attempt < self.config.max_retries:
                await asyncio.sleep(self.config.retry_backoff * (2 ** (attempt - 1)))

        msg = f"远端调用最终失败：{last_err}"
        raise RemoteShareError(msg)
