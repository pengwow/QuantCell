...
class StrategyInfo(BaseModel):
    ...
    file_path: str = Field(
        ..., description="策略文件路径", example="/backend/strategies/sma_cross.py"
    )
    description: str = Field(
        ...,
        description="策略描述",
        example="基于SMA交叉的策略，当短期均线上穿长期均线时买入，下穿时卖出",
    )
    ...
    created_at: datetime = Field(
        ..., description="创建时间", example="2023-01-01T00:00:00"
    )
    updated_at: datetime = Field(
        ..., description="更新时间", example="2023-01-01T00:00:00"
    )
    code: Optional[str] = Field(
        None,
        description="策略代码",
        example="class SmaCross(Strategy):\n    def next(self):\n        self.buy()",
    )
    source: str = Field(..., description="策略来源", example="files")
...
class StrategyExecutionRequest(BaseModel):
    ...
    params: Dict[str, Any] = Field(
        ..., description="策略参数", example={"n1": 10, "n2": 20}
    )
    mode: str = Field(
        ..., description="执行模式", example="backtest", pattern="^(backtest|live)$"
    )
