...
class MeanReversionRL(BaseStrategy):
    ...
    def on_bar(self, bar: dict, ctx: StrategyContext) -> Action:
        ...
        if position > 0 and signal == 1:
            return Action(action_type="buy", confidence=confidence, target_position=position,
                          model_id=model_id, inference_time_us=0)
        if position == 0 and signal == -1:
            return Action(action_type="sell", confidence=confidence, target_position=0.0,
                          model_id=model_id, inference_time_us=0)

        return Action(action_type="hold", confidence=confidence, target_position=0.0,
                      model_id=model_id, inference_time_us=0)
