...
try:
    ...
    from utils.data_utils import get_symbols_from_data_pool, get_source_data_dir
...
def download(
    ...
    示例:
      # 下载所有货币对的1小时数据（最近1个月）
      uv run python -m cli.data download
      
      # 下载指定货币对的1小时数据（最近1个月）
      uv run python -m cli.data download -s BTCUSDT -s ETHUSDT
      
      # 下载指定时间范围的数据
      uv run python -m cli.data download -s BTCUSDT -i 15m --start 20240101 --end 20241231

      # 使用自选组合
      uv run python -m cli.data download --pool 我的自选组合 -i 15m --start 20240101 --end 20241231
    """
    if verbose:
        logger.remove()
        logger.add(sys.stderr, level="DEBUG")
