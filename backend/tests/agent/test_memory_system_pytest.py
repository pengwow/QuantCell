...
from agent.core.memory import MemoryStore, Consolidator, AutoCompact, Dream
...
class TestMemoryStore:
    ...
    def test_read_unprocessed_history(self, store):
        ...
        cursor2 = store.append_history("记录2")
        cursor3 = store.append_history("记录3")
    ...
    def test_raw_archive(self, store):
        """测试原始归档"""
        messages = [
            {"role": "user", "content": "Test message", "timestamp": "2026-04-20 10:00"},
        ]
    ...
    def test_pick_consolidation_boundary(self, consolidator, sessions):
        ...
        for i in range(10):
            ...
            session.messages.append({
                "role": role,
                "content": f"消息 {i}",
                "timestamp": "2026-04-20 10:{:02d}".format(i),
            })
...
class TestAutoCompact:
    ...
    def test_prepare_session(self, auto_compact, sessions):
        ...
        # 无摘要
        result_session, summary = auto_compact.prepare_session(session, "test")
        assert summary is None
