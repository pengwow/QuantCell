# -*- coding: utf-8 -*-
"""
Nautilus Worker 集成测试

测试 Nautilus Worker 的完整集成流程，包括：
- Worker 启动/停止流程
- 策略加载和执行
- 事件同步机制
- 错误恢复
- NautilusWorkerManager 管理器
...
class TestNautilusIntegrationBase:
    """Nautilus 集成测试基类"""
...
class TestWorkerLifecycle(TestNautilusIntegrationBase):
    """Worker 生命周期集成测试"""
...
''')
    ...
    async def test_worker_full_lifecycle(self, mock_trading_node, sample_strategy_file):
        ...
        with patch('worker.nautilus_worker.TradingNode', return_value=mock_trading_node):
            with patch('worker.nautilus_worker.build_nautilus_config', return_value={}):
                # 创建 Worker
                ...
                # 初始化 TradingNode
                node = await worker._init_nautilus_node()
    ...
    async def test_worker_restart_recovery(self, mock_trading_node, sample_strategy_file):
        ...
        with patch('worker.nautilus_worker.TradingNode', return_value=mock_trading_node):
            with patch('worker.nautilus_worker.build_nautilus_config', return_value={}):
                # 第一次启动
                worker1 = NautilusWorkerProcess(
                    worker_id="test-restart",
                    strategy_path=sample_strategy_file,
                    config=config,
                )

                await worker1._init_nautilus_node()
                ...
                # 创建新 Worker 实例（模拟重启）
                worker2 = NautilusWorkerProcess(
                    worker_id="test-restart",
                    strategy_path=sample_strategy_file,
                    config=config,
                )

                await worker2._init_nautilus_node()
...
class TestStrategyExecution(TestNautilusIntegrationBase):
    """策略执行集成测试"""
...
'''
    ...
    async def test_strategy_load_and_execute(self, strategy_file):
        ...
        with patch('worker.nautilus_worker.TradingNode', return_value=mock_node):
            with patch('worker.nautilus_worker.build_nautilus_config', return_value={}):
                worker = NautilusWorkerProcess(
                ...
                # 初始化并启动
                await worker._init_nautilus_node()
                await worker._load_nautilus_strategy()
                await worker._handle_start()

                # 验证策略已加载
                assert worker.nautilus_strategy is not None
...
class TestEventSynchronization(TestNautilusIntegrationBase):
    """事件同步集成测试"""
    ...
    async def test_fill_event_conversion_and_sync(self, mock_comm_client):
        ...
        from worker.nautilus_event_handler import (
            NautilusEventHandler, EventBufferConfig, convert_fill_to_qc
        )
...
class TestErrorRecovery(TestNautilusIntegrationBase):
    """错误恢复集成测试"""
    ...
    async def test_trading_node_init_failure_recovery(self):
        ...
        with patch('worker.nautilus_worker.build_nautilus_config', side_effect=Exception("Config error")):
            worker = NautilusWorkerProcess(
            ...
            result = await worker._init_nautilus_node()
    ...
    async def test_strategy_method_error_handling(self):
        ...
        worker.nautilus_strategy = mock_strategy
...
# =============================================================================
# NautilusWorkerManager 测试
# =============================================================================

class TestNautilusWorkerManager(TestNautilusIntegrationBase):
    """NautilusWorkerManager 集成测试"""
    ...
    async def test_manager_initialization(self, manager_config):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            manager = NautilusWorkerManager(**manager_config)
    ...
    @pytest.mark.asyncio
    async def test_start_and_stop_nautilus_worker(self, manager_config):
        """测试启动和停止 Nautilus Worker"""
        from worker.nautilus_manager import NautilusWorkerManager
...
''')
        ...
        try:
            with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
                with patch('worker.manager.WorkerManager.stop', new_callable=AsyncMock, return_value=True):
                    with patch('worker.nautilus_worker.TradingNode'):
                        with patch('worker.nautilus_worker.build_nautilus_config', return_value={}):
                            manager = NautilusWorkerManager(**manager_config)

                            # 启动 Worker
                            worker_id = await manager.start_nautilus_worker(
                            ...
                            # 停止 Worker
                            result = await manager.stop_nautilus_worker(worker_id)
                            assert result is True
    ...
    async def test_worker_count_limit(self, manager_config):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            ...
            # 尝试启动超过限制的 Worker
            result = await manager.start_nautilus_worker(
                strategy_path="/tmp/test.py",
                config={},
            )
    ...
    def test_merge_config(self, manager_config):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            manager = NautilusWorkerManager(**manager_config)
            manager.nautilus_config = {"global_setting": "value"}
            ...
            assert "nautilus" in merged
            assert merged["nautilus"]["global_setting"] == "value"
...
class TestWorkerMonitorIntegration(TestNautilusIntegrationBase):
    """WorkerMonitor 集成测试"""
    ...
    async def test_monitor_initialization(self, mock_worker_manager):
        """测试监控器初始化"""
        from worker.nautilus_manager import WorkerMonitor
    ...
    async def test_alert_config_management(self, mock_worker_manager):
        """测试告警配置管理"""
        from worker.nautilus_manager import WorkerMonitor, AlertConfig, AlertRule, AlertRuleType, AlertSeverity
    ...
    def test_worker_metrics_dataclass(self):
        ...
        from worker.nautilus_manager import WorkerMetrics
...
class TestEndToEndIntegration(TestNautilusIntegrationBase):
    """端到端集成测试"""
...
''')
                ...
                with patch('worker.manager.WorkerManager.stop', new_callable=AsyncMock, return_value=True):
                    with patch('worker.nautilus_worker.TradingNode', return_value=mock_node):
                        with patch('worker.nautilus_worker.build_nautilus_config', return_value={}):
                            ...
                            # 获取 Worker 状态
                            status = manager.get_nautilus_worker_status(worker_id)
    ...
    async def test_multiple_workers_management(self):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            manager = NautilusWorkerManager(max_workers=5)
            ...
            # 验证 Worker 数量
            assert manager.get_nautilus_worker_count() == 3
