...
from exchange.binance import BinanceConfig, BinanceWebSocketManager
...
class TestWebSocket:
    ...
    async def test_subscribe_kline(self, ws_manager):
        ...
        # 等待接收数据（最多5秒）
        await asyncio.wait_for(self._wait_for_messages(messages, 1), timeout=10.0)
    ...
    async def test_multiple_subscriptions(self, ws_manager):
        ...
        # 等待接收数据
        await asyncio.wait_for(self._wait_for_messages(kline_messages, 1), timeout=10.0)

        await asyncio.wait_for(self._wait_for_messages(trade_messages, 1), timeout=10.0)
