...
def test_reload_remote_config_clears_singleton():
    ...
    with patch(
        ...
        return_value={"share_remote": {"api_key": "qck_v1", "hmac_secret": "s1"}},
    ):
    ...
    with patch(
        "share.config._load_toml_chain",
        return_value={"share_remote": {"api_key": "qck_v2", "hmac_secret": "s2"}},
    ):
...
def test_ensure_remote_credentials_already_ready(monkeypatch):
    ...
    with (
        ...
        patch("share.remote_client.RemoteShareClient") as MockClient,
    ):
