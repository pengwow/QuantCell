...
def test_transform_df_flattens_single_record():
    ...
    assert len(out) == 4
    assert list(out.columns) == ['timestamp', 'symbol', 'side', 'level', 'price', 'quantity']
...
def test_parse_csv_bytes_parses_nested_bids_asks():
    ...
    raw_bytes = (
        ...
        b'1700000000000,BTCUSDT,"[[100.0,1.0],[99.0,2.0]]","[[101.0,1.5],[102.0,2.5]]"\n'
    )
...
def test_collect_data_writes_parquet_with_schema(monkeypatch, tmp_path: Path):
    ...
    assert len(loaded) == 4
    assert list(loaded.columns) == ['timestamp', 'symbol', 'side', 'level', 'price', 'quantity']
