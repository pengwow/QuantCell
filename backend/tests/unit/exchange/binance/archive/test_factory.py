...
def test_create_returns_agg_trades_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.AGG_TRADES, MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_create_returns_trades_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.TRADES, MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_create_returns_book_ticker_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.BOOK_TICKER, MarketType.FUTURES_UM, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_create_returns_book_depth_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.BOOK_DEPTH, MarketType.FUTURES_CM, base_dir=tmp_path, symbol='BTCUSD'
    )
...
def test_create_returns_mark_klines_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.MARK_KLINES, MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_create_returns_index_klines_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.INDEX_KLINES, MarketType.FUTURES_UM, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_create_returns_premium_klines_fetcher(tmp_path: Path):
    f = BinanceArchiveFactory.create(
        ArchiveKind.PREMIUM_KLINES, MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
    )
...
def test_all_21_combinations_return_non_none_instance(tmp_path: Path):
    ...
    for kind in ArchiveKind:
        for market in MarketType:
            f = BinanceArchiveFactory.create(
                kind, market, base_dir=tmp_path, symbol='BTCUSDT'
            )
            assert f is not None, f"Got None for {kind}/{market}"
            assert isinstance(f, _KIND_TO_FETCHER[kind]), (
                f"Wrong class for {kind}/{market}: got {type(f).__name__}"
            )
...
def test_unknown_kind_string_raises_value_error(tmp_path: Path):
    ...
    with pytest.raises(ValueError):
        BinanceArchiveFactory.create(
            'not_a_kind', MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
        )
...
def test_valid_kind_string_also_accepted(tmp_path: Path):
    ...
    f = BinanceArchiveFactory.create(
        'aggTrades', MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
    )
    assert isinstance(f, AggTradesFetcher)
...
def test_each_fetcher_has_required_hooks(tmp_path: Path):
    ...
    for kind in ArchiveKind:
        f = BinanceArchiveFactory.create(
            kind, MarketType.SPOT, base_dir=tmp_path, symbol='BTCUSDT'
        )
        assert f.archive_kind == kind
