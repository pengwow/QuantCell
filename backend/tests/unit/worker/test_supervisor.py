...
class TestRestartPolicy:
    ...
    def test_custom_values(self):
        """测试自定义值"""
        policy = RestartPolicy(max_restarts=5, restart_window=600, backoff_base=2.0, backoff_max=120.0)
...
class TestHealthCheckConfig:
    ...
    def test_custom_values(self):
        """测试自定义值"""
        config = HealthCheckConfig(heartbeat_timeout=60, check_interval=20, unhealthy_threshold=5)
