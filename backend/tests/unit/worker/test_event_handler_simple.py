...
class TestSimpleEventHandler:
    ...
    def event_config(self):
        return SimpleEventBufferConfig(buffer_size=10, flush_interval=0.1, batch_size=3)
    ...
    def event_handler(self, event_config, mock_comm_client):
        return SimpleEventHandler(worker_id=1, comm_client=mock_comm_client, config=event_config)
