...
def memory_limit(max_size_mb: int = 200):
    ...
    if end_mem - start_mem > max_size_mb:
        msg = f"内存使用超过限制: {end_mem - start_mem:.2f}MB > {max_size_mb}MB"
...
_mock_service_module.get_worker_status = AsyncMock(
    return_value={"worker_id": 1, "status": "running", "is_healthy": True}
)
_mock_service_module.health_check = AsyncMock(return_value={"worker_id": 1, "is_healthy": True})
_mock_service_module.get_worker_metrics = AsyncMock(
    return_value={
        "worker_id": 1,
        "network_in": 1024000,
        "network_out": 512000,
...
_mock_service_module.deploy_strategy = AsyncMock(return_value={"deployed": True, "strategy_id": 1, "worker_id": 1})
_mock_service_module.undeploy_strategy = AsyncMock(return_value={"undeployed": True, "worker_id": 1})
...
_mock_service_module.send_trading_signal = AsyncMock(return_value={"sent": True, "signal_id": "test-signal-id"})
_mock_service_module.stream_logs = AsyncMock()
_mock_service_module.batch_operation = AsyncMock(return_value={"success": [1, 2], "failed": {}, "total": 2})
...
def _install_axon_events_stub() -> None:
    ...
    if "axon_quant.core" in sys.modules and hasattr(sys.modules["axon_quant.core"], "events"):
        return
    ...
    for cls in (OrderEvent, OrderAccepted, OrderCanceled, OrderRejected, OrderFilled):
        setattr(events_module, cls.__name__, cls)
    core_module.events = events_module
...
def mock_worker_service():
    ...
    mock.get_worker_status = AsyncMock(return_value={"worker_id": 1, "status": "running", "is_healthy": True})
    mock.health_check = AsyncMock(return_value={"worker_id": 1, "is_healthy": True})
    return mock
