...
def test_trend_follow_buy_on_breakout():
    s = TrendFollow(StrategyConfig(name="trend_follow", params={"lookback": 5, "atr_period": 3, "atr_mult": 5.0}))
    ctx = StrategyContext(symbol="BTCUSDT")
    # 5 根横盘 + 突破大涨
...
def test_mean_reversion_buy_on_oversold():
    s = MeanReversion(StrategyConfig(name="mean_reversion", params={"bb_period": 5, "rsi_period": 5, "std_mult": 0.5}))
    ctx = StrategyContext(symbol="BTCUSDT")
    # 横盘后暴跌 → 触发 buy
