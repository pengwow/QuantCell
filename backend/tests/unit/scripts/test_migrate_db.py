...
class TestGetDbPath:
    ...
    def test_get_db_path_found(self, tmp_path):
        """测试找到数据库文件"""
        from cli.migrate import get_db_path

        db_file = tmp_path / "data" / "quantcell_sqlite.db"
        db_file.parent.mkdir(parents=True)
        db_file.write_text("")

        with patch("cli.migrate.Path") as mock_path_cls:
    ...
    def test_get_db_path_not_found(self, tmp_path):
        ...
        with patch("cli.migrate.Path", return_value=mock_instance), pytest.raises(FileNotFoundError):
            get_db_path()
...
class TestCheckColumnExists:
    ...
    def test_column_exists(self):
        """测试列已存在的情况"""
        from cli.migrate import check_column_exists
...
class TestMigrateStrategiesTable:
    ...
    def test_migrate_success(self):
        """测试迁移成功的情况"""
        from cli.migrate import migrate_strategies_table

        mock_conn = MagicMock()

        with patch("cli.migrate.check_column_exists", return_value=False):
    ...
    def test_migrate_skip_existing(self):
        ...
        with patch("cli.migrate.check_column_exists", return_value=True):
            result = migrate_strategies_table(mock_conn)
...
class TestMain:
    ...
    def test_main_success(self, mock_connect, mock_get_db_path):
        ...
        with patch("cli.migrate.migrate_strategies_table", return_value=True):
            result = main()
    ...
    def test_main_no_migration_needed(self, mock_connect, mock_get_db_path):
        ...
        with patch("cli.migrate.migrate_strategies_table", return_value=False):
            result = main()
