...
class TestFormatSize:
    ...
    def test_format_size_bytes(self):
        """测试字节级别"""
        from scripts.data_cli import format_size
...
class TestCalculateDataCompleteness:
    ...
    def test_completeness_no_data(self):
        """测试无数据情况"""
        from scripts.data_cli import calculate_data_completeness
...
class TestFormatCompleteness:
    ...
    def test_format_completeness_good(self):
        """测试良好完整度"""
        from scripts.data_cli import format_completeness
...
class TestValidateParquetExport:
    ...
    def test_validate_row_count_mismatch(self):
        ...
        with patch("scripts.data_cli.load_from_parquet", return_value=loaded_df):
    ...
    def test_validate_success(self):
        ...
        with patch("scripts.data_cli.load_from_parquet", return_value=original_df.copy()):
            result = _validate_parquet_export(mock_path, original_df)
...
class TestFindParquetFile:
    ...
    def test_find_parquet_file_spot(self):
        ...
        with patch("scripts.data_cli.get_source_data_dir", return_value=Path("/data")):
...
class TestGetDefaultDateRange:
    ...
    def test_default_date_range(self):
        """测试默认日期范围"""
        from scripts.data_cli import _get_default_date_range
...
class TestScanParquetFiles:
    ...
    def test_scan_empty_directory(self):
        ...
        with patch("pathlib.Path.iterdir", return_value=[]):
            with patch("scripts.data_cli.get_source_data_dir", return_value=Path("/empty")):
    ...
    def test_scan_with_files(self):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch("pathlib.Path.iterdir", return_value=[mock_interval_dir]):
                with patch("scripts.data_cli.get_parquet_info", return_value={"num_rows": 100, "file_size_bytes": 1024}):
                    with patch("scripts.data_cli.get_source_data_dir", return_value=Path("/data")):
...
class TestDataCliCommands:
    ...
    @patch("scripts.data_cli._find_parquet_file")
    @patch("scripts.data_cli.load_from_parquet")
    def test_export_csv_file_not_found(self, mock_load, mock_find):
        ...
        result = runner.invoke(app, ["export", "csv", "--symbol", "BTCUSDT", "--interval", "1h", "--output", "test.csv"])
    ...
    @patch("scripts.data_cli.task_manager")
    ...
    def test_status_task_found(self, mock_task_manager):
        ...
        mock_task_manager.get_task.return_value = {
            ...
            "status": "completed",
            "task_type": "download",
            "progress": {"percentage": 100.0, "completed": 10, "total": 10, "failed": 0},
    ...
    def test_list_symbols_no_data(self):
        ...
        with patch("scripts.data_cli.scan_parquet_files", return_value=[]):
    ...
    @patch("scripts.data_cli.scan_parquet_files")
...
class TestImportCsv:
    ...
    @patch("scripts.data_cli.pd.read_csv")
    @patch("scripts.data_cli.Path")
...
class TestQualityCommands:
    ...
    @patch("scripts.data_cli.typer.secho")
...
class TestFormatTimeRange:
    ...
    def test_format_time_range_none(self):
        """测试无时间范围"""
        from scripts.data_cli import format_time_range
