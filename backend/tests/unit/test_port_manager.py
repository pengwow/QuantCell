...
class TestZombieProcessDetection:
    ...
    def test_detect_zombie_process_success(self, mock_run, temp_config_dir):
        ...
        with (
            patch.object(PortManager, "_is_zombie_process", return_value=True),
    ...
    def test_is_zombie_process_detection(self, mock_run, temp_config_dir):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="/usr/bin/python main:app --host 0.0.0.0 --port 8000")
    ...
    def test_is_not_zombie_process(self, mock_run, temp_config_dir):
        ...
        mock_run.return_value = MagicMock(returncode=0, stdout="/usr/bin/nginx -g daemon off;")
...
class TestEdgeCasesAndValidation:
    ...
    def test_validate_config_valid_data(self, temp_config_dir):
        ...
        valid_config = {"version": "1.0", "services": {"fastapi": {"port": 8000}}}
...
class TestPortAllocationError:
    ...
    def test_exception_attributes(self, temp_config_dir):
        """异常包含正确的属性"""
        error = PortAllocationError("测试错误消息", service_name="test_service", port=8080)
