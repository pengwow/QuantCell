...
class TestTradingPerformanceBase:
    """交易性能测试基类"""
...
class TestOrderLatency(TestTradingPerformanceBase):
    """订单延迟性能测试"""
    ...
    def test_order_conversion_latency(self, benchmark):
        ...
        result = benchmark(convert_order_to_trading, qc_order)
        assert result is not None

    @pytest.mark.benchmark
    def test_fill_event_conversion_latency(self, benchmark):
        """测试成交事件转换延迟"""
        from worker.event_handler import convert_fill_to_qc
...
class TestEventThroughput(TestTradingPerformanceBase):
    """事件处理吞吐量测试"""
    ...
    def test_position_conversion_throughput(self, benchmark):
        ...
        from trading_engine.model.enums import PositionSide
        from decimal import Decimal
...
class TestMemoryUsage(TestTradingPerformanceBase):
    """内存使用测试"""
...
class TestConcurrentWorkerPerformance(TestTradingPerformanceBase):
    """并发 Worker 性能测试"""
    ...
    async def test_concurrent_worker_operations(self):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            manager = TradingNodeWorkerManager(max_workers=10)
...
class TestConfigBuildPerformance(TestTradingPerformanceBase):
    """配置构建性能测试"""

    @pytest.mark.benchmark
    def test_build_trading_config_performance(self, benchmark):
        """测试交易配置构建性能"""
        ...
        result = benchmark(build_trading_config, config)
        assert result is not None
...
class TestDataConversionPerformance(TestTradingPerformanceBase):
    """数据转换性能测试"""
    ...
    def test_tick_conversion_performance(self, benchmark):
        ...
        from trading_engine.model.data import QuoteTick
        ...
        def convert_multiple():
            with patch('strategy.trading_adapter.TradingQuoteTick', QuoteTick):
                for _ in range(1000):
    ...
    def test_position_event_conversion_performance(self, benchmark):
        """测试持仓事件转换性能"""
        from worker.event_handler import convert_position_event_to_qc
        from trading_engine.model.events.position import PositionChanged
...
class TestEndToEndPerformance(TestTradingPerformanceBase):
    """端到端性能测试"""
    ...
    async def test_full_event_pipeline(self):
        ...
        from worker.event_handler import (
            EventHandler, EventBufferConfig,
            convert_fill_to_qc, convert_order_event_to_qc
        )
        from trading_engine.execution.messages import OrderFilled, OrderAccepted
    ...
    async def test_worker_manager_scalability(self):
        ...
        with patch('worker.manager.WorkerManager.start', new_callable=AsyncMock, return_value=True):
            manager = TradingNodeWorkerManager(max_workers=20)
