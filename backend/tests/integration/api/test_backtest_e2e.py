...
class TestBacktestE2E:
    ...
    def test_backtest_api_all_templates_load_success(self, client: TestClient, mocker):
        ...
        """
        strategy_templates = [
            "dual_ma", "trend_follow", "momentum", "grid",
            "mean_reversion", "sma_crossover", "funding_arbitrage", "llm_signal"
        ]
        ...
        for strategy_name in strategy_templates:
            ...
            assert response.status_code == 200, \
                f"策略 {strategy_name} 加载失败: {response.text}"
            data = response.json()
            assert data["code"] == 0, \
                f"策略 {strategy_name} 返回错误: {data.get('message')}"
