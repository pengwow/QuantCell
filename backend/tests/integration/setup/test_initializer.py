...
from utils.initializer import InitializationStep, ProjectInitializer
...
class TestInitializationStep:
    ...
    def test_step_execution_success(self):
        ...
        step = InitializationStep(name="test", description="Test Step", func=mock_func)
    ...
    def test_step_skip(self):
        """测试步骤跳过"""
        step = InitializationStep(name="test", description="Test Step", func=Mock())
...
class TestProjectInitializer:
    ...
    @patch("utils.config_manager.ConfigManager.import_from_toml")
            ...
            with patch.object(initializer, "_step_db_migrate", return_value=True):
                with patch.object(initializer, "_step_data_seed", return_value=True):
                    with patch.object(initializer, "_step_verification", return_value=True):
                        results = initializer.run(config_path="test.toml", skip_config_import=False)
