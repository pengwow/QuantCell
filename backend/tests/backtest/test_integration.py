...
@pytest.fixture(scope="function")
def default_engine(default_engine_config: dict[str, Any]) -> Generator[Engine]:
    """
    创建并管理 Engine 实例
...
@pytest.fixture(scope="function")
def legacy_engine(legacy_engine_config: dict[str, Any]) -> Generator[LegacyEngine]:
    """
...
class TestMultiCurrencyBacktest:
    ...
    def test_multi_symbol_config_legacy(self):
        ...
        engine = LegacyEngine(config)
        assert engine.config["backtest_config"]["symbols"] == [
            "BTCUSDT",
            "ETHUSDT",
            "ADAUSDT",
        ]
