...
class MockJWTToken:
    ...
    def create_near_expiration_token(cls, minutes_left: int = 5) -> str:
        ...
        Returns:
            str: 即将过期的JWT令牌
        """
        expire = datetime.now(UTC) + timedelta(minutes=minutes_left)
...
class MockAuthService:
    ...
    def validate_user(self, user_id: str) -> dict[str, Any] | None:
        """
        验证用户是否存在
...
def mock_jwt_auth_required(mocker, user_id: str = "test_user_123"):
    ...
    Returns:
        Mock: mock对象
    """
    mock_payload = {"sub": user_id, "name": "test_user"}

    return mocker.patch("utils.auth.decode_jwt_token", return_value=mock_payload)
...
def mock_get_current_user(mocker, user_id: str = "test_user_123"):
    ...
    Returns:
        Mock: mock对象
    """
    mock_user = {"sub": user_id, "name": "test_user"}

    return mocker.patch("utils.auth.get_current_user", return_value=mock_user)
