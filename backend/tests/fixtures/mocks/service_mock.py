...
class MockStrategyService:
    ...
    def get_strategy_list(self, source: str | None = None) -> list[dict[str, Any]]:
        """获取策略列表"""
    ...
    def get_strategy_detail(self, strategy_name: str, file_content: str | None = None) -> dict[str, Any] | None:
        """获取策略详情"""
...
class MockBacktestService:
    ...
    def run_backtest(self, strategy_config: dict[str, Any], backtest_config: dict[str, Any]) -> dict[str, Any]:
        """执行回测"""
        task_id = f"bt_{hash(str(strategy_config) + str(backtest_config))}"
        return {"task_id": task_id, "status": "completed", "message": "回测完成"}
...
class MockSystemConfig:
    ...
    @classmethod
    def get_with_details(cls, key: str) -> dict[str, Any] | None:
        """获取配置详情"""
        config = cls._configs.get(key)
        if config and config.get("is_sensitive"):
            config = config.copy()
