...
from exchange import ConnectionStatus, ConnectionTestResult, Exchange
...
class BinanceExchange(Exchange):
    ...
    def _patch_load_markets(self) -> None:
        ...
        def patched_load_markets(self_inner, reload=False, params=None):
    ...
    def test_connection(self) -> ConnectionTestResult:
        ...
        if self.testnet:
            ...
            if not testnet_url and is_futures:
                testnet_url = self.BINANCE_TESTNET_URLS.get("future", "")
            base_url = testnet_url + "/fapi/v1" if is_futures else testnet_url + "/api/v3"
        else:
            base_url = "https://fapi.binance.com/fapi/v1" if is_futures else "https://api.binance.com/api/v3"

        logger.info(
            f"测试连接开始: exchange={self.exchange_name}, mode={self.trading_mode}, testnet={self.testnet}, base_url={base_url}"
        )
    ...
    def _test_auth_direct(self, base_url: str, details: dict) -> dict:
        ...
        try:
            ...
            if is_futures:
                ...
                url = f"{api_base}/balance" if self.testnet else f"{api_base.rsplit('/v1', 1)[0]}/v2/balance"
            else:
                # spot
                api_base = base_url.rstrip("/")
            ...
            if "application/json" not in content_type:
                ...
                return {
                    "success": False,
                    "error": f"API返回非JSON内容 ({resp.status_code}): {raw_text}",
                }
            ...
            if "-2015" in error_msg:
                hint = (
                    ...
                    "① API Key 是否勾选与当前测试一致的业务；"
                    "② 若启用 IP 白名单，是否包含当前服务器出口 IP；"
                    "③ 测试网模式必须使用在 testnet.binance.vision 申请的专用 API Key；"
                    "④ 无多余空格、复制完整 Secret。"
                )
                if self.testnet:
