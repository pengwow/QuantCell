...
class BinanceClient:
    ...
    def get_account(self) -> dict[str, Any]:
        ...
        try:
            ...
            raise BinanceAPIError(msg, code=e.code)

    def get_balance(self, asset: str | None = None) -> list[AccountBalance]:
        """
        获取账户余额
    ...
    def get_order_book(self, symbol: str, limit: int = 100) -> dict[str, Any]:
        ...
        try:
            ...
            msg = f"Failed to get order book: {e}"
            raise BinanceAPIError(msg, code=e.code)
    ...
    def get_recent_trades(self, symbol: str, limit: int = 500) -> list[dict[str, Any]]:
        ...
        try:
            ...
            msg = f"Failed to get recent trades: {e}"
            raise BinanceAPIError(msg, code=e.code)
    ...
    def create_order(self, order: OrderRequest) -> OrderResponse:
        ...
        try:
            ...
            response = (
                self._client.order_limit_buy(**params)
                if order.order_type.value == "LIMIT" and order.side.value == "BUY"
                else self._client.order_limit_sell(**params)
                if order.order_type.value == "LIMIT" and order.side.value == "SELL"
                else self._client.order_market_buy(**params)
                if order.order_type.value == "MARKET" and order.side.value == "BUY"
                else self._client.order_market_sell(**params)
            )
    ...
    def get_open_orders(self, symbol: str | None = None) -> list[dict[str, Any]]:
        ...
        try:
            ...
            msg = f"Failed to get open orders: {e}"
            raise BinanceAPIError(msg, code=e.code)
    ...
    def get_all_orders(
        ...
        try:
            ...
            msg = f"Failed to get all orders: {e}"
            raise BinanceAPIError(msg, code=e.code)
    ...
    def _handle_error(self, error: Exception, operation: str):
        ...
        if isinstance(error, BinanceAPIException):
            if error.code == -2015:  # Invalid API-key, IP, or permissions
                ...
                raise BinanceAuthenticationError(msg)
            elif error.code == -1003:  # Too many requests
                msg = f"Rate limit exceeded: {error}"
                raise BinanceRateLimitError(msg)
            else:
                msg = f"{operation} failed: {error}"
                raise BinanceAPIError(msg, code=error.code)
