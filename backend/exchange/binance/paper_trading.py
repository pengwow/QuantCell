...
class PaperTradingAccount:
    ...
    def create_order(
        ...
        if not symbol or not isinstance(symbol, str):
            msg = "Invalid symbol"
            raise BinanceOrderError(msg, symbol=symbol)
    ...
    def get_open_orders(self, symbol: str | None = None) -> list[PaperOrder]:
        """获取未成交订单"""
        orders = [
            order for order in self.orders.values() if order.status in [OrderStatus.NEW, OrderStatus.PARTIALLY_FILLED]
        ]
    ...
    def _update_position(self, symbol: str, side: OrderSide, quantity: float, price: float):
        ...
        if side == OrderSide.BUY:
            if position and position.side == "LONG":
                ...
                # 新开多头持仓
                self.positions[symbol] = PaperPosition(symbol=symbol, quantity=quantity, avg_price=price, side="LONG")
        else:  # SELL
            if position and position.side == "SHORT":
                ...
                # 新开空头持仓
                self.positions[symbol] = PaperPosition(symbol=symbol, quantity=quantity, avg_price=price, side="SHORT")
